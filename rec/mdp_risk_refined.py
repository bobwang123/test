...
def value_iteration(num_city, util_amplifier, epslon, alpha, gamma, reward,
    ...
    while error > epslon and count < max_iter:
        ...
        error = np.sqrt(sum(sum(np.square(new_utility - utility).reshape(1, num_city * num_city * num_city))))
...
# utility = np.zeros([num_city, num_city, num_city])
